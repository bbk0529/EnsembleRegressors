...
class DistanceMatrix() : 
    ...
    def __init__(self, data: np.ndarray, n: int, radius: float) : 
        self.data = data
        ...
        self._setup_parameters(self._max_radius)
    ...
    def _setup_parameters(self, max_radius) : 
    ...
    def print_stats(self, options: list = [True,True,True,True]) : 
        ...
        if options[0] : 
            ...
            print("\tmax radius {}".format('self._max_radius'))
...
def test(N, n,radius) : 
    ...
    dm = DistanceMatrix(data, n, radius)         
    ...
    filename = 'test_result_w_max_radius' + str(radius) + '.txt'
    f = open(filename, 'a')    
