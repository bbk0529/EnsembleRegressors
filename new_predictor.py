...
class SmoothingAndPredict(Classifier):    
    def correct(self, X_raw, y_raw, graph=False) : 
        ...
        while True :  
            ...
            y[idx] = pred[idx] 
            if graph :                 
                plt.plot(y, color='red', linewidth=3, alpha=0.8, linestyle='--')
...
class Data(ABC):
    def __init__(self, p_noise_stations, p_noise_timesteps, min_noises, max_noises):
        ...
        self.ts_data, self.lst_station_timestep = self.add_noise(
            self.ts_rawdata, p_noise_stations=p_noise_stations, p_noise_timesteps = p_noise_timesteps, min_noises=min_noises, max_noises=max_noises, both_side = True
        )
