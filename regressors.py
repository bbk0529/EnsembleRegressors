...
class Data(ABC):
    ...
    def add_noise3(self, ts_data, p_noise_stations: float, p_noise_timesteps: float) :
        ...
        picked_stations = np.random.choice(range(n_stations), size= round(p_noise_stations * n_stations))
        ...
        for s in picked_stations : 
            ...
            matrix_noises[s, pick_timesteps ] = np.random.rand(len(pick_timesteps)) * 10 
